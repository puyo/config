...
Plug 'chriskempson/vim-tomorrow-theme' " colour scheme
Plug 'colepeters/spacemacs-theme.vim' " colour scheme
Plug 'tomasr/molokai' " colour scheme
Plug 'zefei/cake16' " colour scheme
Plug 'puyo/vim-colors-pencil-warm' " colour scheme
"Plug 'puyo/vim-colors-pencil-warm' " colour scheme
Plug 'vim-scripts/CycleColor' " Cycle themes

" files and buffers
Plug 'kien/ctrlp.vim' " jump to files: in directory tree, most-recently-used, open buffers
Plug 'rgarver/Kwbd.vim' " kill buffers without closing their window

" text objects
Plug 'kana/vim-textobj-user' " allows custom text objects
Plug 'michaeljsmith/vim-indent-object' " 'i' is a text object that selects the current indentation level. e.g. vii
Plug 'nelstrom/vim-textobj-rubyblock' " 'r' is a text object that selects the current Ruby structure. e.g. vir
Plug 'wellle/targets.vim' " Argument text objects - ia, aa
Plug 'tommcdo/vim-exchange' " Exchange text objects

" tpope selection
Plug 'tpope/vim-commentary' " gc to toggle comments
Plug 'tpope/vim-eunuch' " UNIX commands :Unlink :Remove :Move :Chmod :Find :Locate :SudoWrite :W
Plug 'tpope/vim-fugitive' " Git commands :Ggrep
Plug 'tpope/vim-rhubarb' " GitHub commands :GBrowse
Plug 'tpope/vim-repeat' " press . to repeat more sophisticated things
Plug 'tpope/vim-surround' " Edit quotes and brackets more easily
Plug 'tpope/vim-scriptease' " for working on viml

" projects
Plug 'tpope/vim-projectionist' " Project mode
Plug 'airblade/vim-rooter' " Find project root and auto change directory to it
...
Plug 'ervandew/supertab' " press <Tab> to complete the current word

Plug 'osyo-manga/vim-over' " Version of :%s that previews that changes that will be made
...
set synmaxcol=300 " faster syntax highlighting
...
set wildignore+=*.dll,*.o,*.obj,*.bak,*.exe,*.pyc,*.jpg,*.gif,*.png,*~,*.swp " ignore these
...
inoremap <D-/> <ESC>lma:Commentary<CR>`ai
nnoremap <D-/> :Commentary<CR>
vnoremap <D-/> :Commentary<CR>gv
